    ...
    <AppLayout>
      <div className="max-w-2xl space-y-5">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => router.back()} className="text-slate-400 hover:text-white transition-colors bg-white/10 p-2 rounded-full">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h1 className="text-2xl font-black text-white">Add Tasks</h1>
            <p className="text-sm text-slate-300 font-bold">Type naturally — AI extracts and classifies each task (FR-1, FR-2)</p>
          </div>
        </div>
        ...
        {overwhelmedState === 'overwhelmed' ? (
          <div className="glass-panel p-6 text-center border border-white/5">
            ...
            <h2 className="font-bold text-white mb-2">Rest Mode Active</h2>
            <p className="text-sm text-slate-300">Adding new tasks is limited during rest mode. Take a break first.</p>
          </div>
        ) : (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={mc} className="glass-panel p-6 border border-white/5 shadow-2xl">
            <div className="flex items-center gap-2 mb-4">
              <Zap className="w-5 h-5 text-sky-400" />
              <h2 className="font-bold text-white">Natural Language Input</h2>
            </div>
            <textarea
              autoFocus
              value={input}
              ...
              className="input-skeu w-full bg-black/30 rounded-xl p-4 text-sm text-white placeholder:text-slate-500 resize-none border border-white/10 focus:outline-none focus:ring-2 focus:ring-sky-400 min-h-[100px]"
              rows={4}
            />
            {error && (
              <p className="text-xs text-amber-400 mt-2 flex items-center gap-1.5 font-bold">
                <AlertTriangle className="w-3.5 h-3.5" /> {error}
              </p>
            )}
            <div className="flex justify-between items-center mt-3">
              <p className="text-xs text-slate-400 font-bold italic">Ctrl+Enter to extract · gemini-3-flash + Zod schema</p>
              <button
                onClick={handleExtract}
                disabled={isExtracting || !input.trim()}
                className="glow-button text-white font-black px-5 py-2 rounded-xl text-sm flex items-center gap-2 disabled:opacity-50"
              >
        ...
        <AnimatePresence>
          {preview && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={mc}
              className="glass-panel p-6 border border-white/5 shadow-2xl"
            >
              ...
              <div className="space-y-3">
                {preview.map((task, i) => {
                  ...
                  return (
                  <div key={i} className={`bg-black/30 rounded-2xl border border-white/10 overflow-hidden ${cls.bg.replace('bg-', 'border-l-4 border-l-')}`}>
                    {/* Task row */}
                    <div className="flex items-center gap-3 p-4">
                      <CheckCircle className="w-4 h-4 text-emerald-400 shrink-0" />
                      <span className="flex-1 font-bold text-white text-sm">{task.name}</span>
                      <div className="flex items-center gap-1.5 flex-wrap justify-end">
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-black ${cls.bg} ${cls.text} badge-skeu shrink-0 border border-white/10 shadow-sm`}>{cat?.emoji ?? '📌'} {task.category}</span>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-black ${DEMAND_COLORS[task.demand_type]} border border-white/5 shadow-sm`}>{task.demand_type}</span>
                    ...
                    <div className="px-4 pb-3 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-white/5 pt-2">
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="input-skeu text-[10px] bg-black/20 border border-white/10 rounded-lg px-2 py-1 text-white focus:outline-none"
                        />
                      </div>
                      
                      <div className="flex items-center gap-2">
                        <RefreshCw className="w-3.5 h-3.5 text-slate-400" />
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Repeat:</span>
                      ...
                      {task.recurring === 'daily' && (
                        <div className="flex items-center gap-2 ml-2">
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Times/day:</span>
                          <input
                            type="number"
                            min="1"
                            max="10"
                            ...
                            className="input-skeu w-10 text-[10px] bg-black/20 border border-white/10 rounded-lg px-2 py-1 text-white focus:outline-none"
                          />
                        </div>
                      )}
                    </div>
              ...
              <div className="flex gap-3 mt-5">
                {saved ? (
                  ...
                  <>
                    <button onClick={confirm} className="glow-button text-white font-black px-6 py-2.5 rounded-xl text-sm flex items-center gap-2">
                      <Plus className="w-4 h-4" /> Save {preview.length} Task{preview.length !== 1 ? 's' : ''}
                    </button>
                    <button onClick={() => { setPreview(null); setInput('') }} className="bg-black/40 border border-white/5 text-slate-400 font-bold px-4 py-2.5 rounded-xl text-sm hover:bg-black/60 hover:text-white transition-all">
                      Discard
                    </button>
                  </>
                )}
